...
def get_options():
    ...
    if not (configs.interface or configs.config_file):
        # If CLI is being used, check for interface (mandatory). Otherwise, check if config_file was provided.
        # If neither is provided, stop execution.
        return False

    # If config is provided, at this moment, its content is used instead of CLI.
    # That happens because the config file supports sections, or multiple instances while CLI supports a single
